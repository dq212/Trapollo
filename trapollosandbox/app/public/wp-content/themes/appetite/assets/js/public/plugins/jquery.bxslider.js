  ...
  $.fn.bxSlider = function(options) {
    ...
    var init = function init() {
      ...
      if (slider.children.length < slider.settings.maxSlides)
        slider.settings.maxSlides = slider.children.length; // if random start, set the startSlide setting to random number
      ...
      slider.carousel =
        slider.settings.minSlides > 1 || slider.settings.maxSlides > 1; // if carousel, force preloadImages = 'all'
      ...
      slider.maxThreshold =
        ...
        (slider.settings.maxSlides - 1) * slider.settings.slideMargin; // store the current state of the slider (if currently animating, working is true)
      ...
      if (slider.settings.mode == 'vertical')
        slider.settings.maxSlides = slider.settings.minSlides; // save original style data
    ...
    var setup = function setup() {
      ...
      if (slider.settings.mode == 'vertical' && slider.settings.slideMargin > 0)
        slider.children.css('marginBottom', slider.settings.slideMargin); // if "fade" mode, add positioning and z-index CSS
      ...
      if (slider.settings.preloadImages == 'all')
        preloadSelector = slider.children; // only check for control addition if not in "ticker" mode
      ...
      if (!slider.settings.ticker) {
        ...
        if (slider.settings.auto && slider.settings.autoControls)
          appendControlsAuto(); // if any control option is requested, add the controls wrapper
    ...
    var start = function start() {
      ...
      if (slider.settings.mode == 'vertical')
        slider.settings.adaptiveHeight = true; // set the viewport height
      ...
      if (
        ...
        initAuto(); // if ticker is true, start the ticker
        ...
        if (!slider.carousel) {
          ...
          var currentIndex =
            slider.settings.moveSlides == 1
              ? slider.active.index
              : slider.active.index * getMoveBy(); // add the current slide to the children
      ...
      if (slider.usingCSS) {
        ...
        var propValue =
          slider.settings.mode == 'vertical'
            ...
            : 'translate3d(' + value + 'px, 0, 0)'; // add the CSS transition-duration
    ...
    var appendControls = function appendControls() {
      ...
      if (!slider.settings.nextSelector && !slider.settings.prevSelector) {
        ...
        slider.controls.directionEl
          ...
          .append(slider.controls.next); // slider.viewport.append(slider.controls.directionEl);
      ...
      if (slider.settings.tickerHover && !slider.usingCSS) {
        ...
        slider.viewport.hover(
          ...
          function() {
            ...
            var property =
              slider.settings.mode == 'horizontal' ? 'left' : 'top'; // calculate the new speed
    ...
    var tickerLoop = function tickerLoop(resumeSpeed) {
      ...
      if (slider.settings.autoDirection == 'next') {
        position = el
          .find('.bx-clone')
          .first()
          .position(); // if "prev" animate left position to 0, then reset left to first non-clone child
      } else {
        ...
        if ('undefined' !== typeof position) {
          var value =
            slider.settings.mode == 'horizontal'
              ...
              : -position.top; // plugin values to be animated
