...
function appetite_get_primary_header_image_url( $type = false, $options = false ) {
	...
	if ( ! empty( $type ) ) {
		...
		$the_ID = in_the_loop() ? get_the_ID() : get_queried_object_id();

		if ( has_post_thumbnail( $the_ID ) ) {
			$image_url = esc_url( wp_get_attachment_url( get_post_thumbnail_id( $the_ID ) ) );
		}
	}
...
/**
 * Adds a title attribute to the social menu link.
 */
function appetite_nav_menu_link_attributes( $atts, $item, $args ) {
...
add_filter( 'nav_menu_link_attributes', 'appetite_nav_menu_link_attributes', 10, 3 );
 ...
 */
function appetite_page_templates( $templates ) {
	// Hide the Menu template if needed.
	if ( ! apply_filters( 'appetite_is_jetpack_nova_cpt', true ) ) {
		unset( $templates['templates/menu-page.php'] );
	}

	return $templates;
}
add_filter( 'theme_page_templates', 'appetite_page_templates' );
