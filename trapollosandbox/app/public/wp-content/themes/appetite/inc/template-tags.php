 ...
 */
function appetite_posted_on( $the_ID = null ) {
	if ( ! isset( $the_ID ) ) {
		$the_ID = get_the_ID();
	} else {
		$the_ID = absint( $the_ID );
	}

	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U', $the_ID ) !== get_the_modified_time( 'U', $the_ID ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}

	$time_string = sprintf( $time_string,
		esc_attr( get_the_date( DATE_W3C, $the_ID  ) ),
		esc_html( get_the_date( '', $the_ID ) ),
		esc_attr( get_the_modified_date( DATE_W3C, $the_ID  ) ),
		esc_html( get_the_modified_date( '', $the_ID ) )
	);

	$posted_on = sprintf(
		'%s',
		'<a href="' . esc_url( get_permalink( $the_ID ) ) . '" rel="bookmark">' . $time_string . '</a>'
	);
...
function appetite_entry_footer() {
	...
	if ( $tags_list && ! is_wp_error( $tags_list ) ) {
		printf( '<div class="tags-links">' . esc_html__( 'Tagged: %s', 'appetite' ) . '</div>', $tags_list );
	}
}
endif;
